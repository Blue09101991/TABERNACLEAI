...
import { CopyIcon, DownloadIcon, SettingsIcon, StarIcon, DeleteIcon } from '@chakra-ui/icons';
import axios from 'axios';
import Modal from 'react-modal';
import { saveFavoriteImage, fetchFavoriteImages, deleteFavoriteImage } from '../../config/firebase'; // Adjust the import path as needed
...
const predefinedImages: ImageOption[] = [
  ...
  { src: '/img/imggeneration/33.jpeg', title: 'Avatar', prompt: 'Generate a portrait of a {inputPrompt} with a confident and cheerful expression. They have long, flowing hair and are wearing casual yet stylish clothing. The background features a vibrant cityscape with bustling streets and colorful buildings.' },
  // Add more predefined images and their prompts here
];

Modal.setAppElement('#__next'); // Ensure to set the app element for accessibility
...
export default function ImageGeneration() {
  ...
  const [imageGenerationCost, setImageGenerationCost] = useState(10);
  ...
  useEffect(() => {
    if (user && user.email) {
      fetchFavoriteImages(user.email).then(images => setFavoriteImages(images));
    }
  }, [user]);

  useEffect(() => {
    fetchImageGenerationCost();
  }, []);

  const fetchImageGenerationCost = async () => {
    try {
      const docRef = doc(firestore, 'price', 'credits');
      const docSnap = await getDoc(docRef);
      if (docSnap.exists()) {
        const data = docSnap.data();
        console.log("ImageGeneration:", data.imageGeneration);
        setImageGenerationCost(data.imageGeneration); // Default to 10 if value not found
      } else {
        console.log('No such document!');
      }
    } catch (error) {
      console.error('Error fetching image generation cost:', error);
    }
  };
  ...
  const handleGenerateImage = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setUsage(imageGenerationCost * numberOfImages);
  ...
  const handleFavoriteImage = async (image: string) => {
    if (user && user.email) {
      await saveFavoriteImage(user.email, image);
      const newFavoriteImages = await fetchFavoriteImages(user.email);
      setFavoriteImages(newFavoriteImages);
      toast({
        title: 'Image added to favorites',
        status: 'success',
        duration: 2000,
        isClosable: true,
      });
  ...
  const handleDeleteFavoriteImage = async (docId: string, storagePath: string) => {
    if (user && user.email) {
      await deleteFavoriteImage(docId, storagePath);
      const newFavoriteImages = await fetchFavoriteImages(user.email);
      setFavoriteImages(newFavoriteImages);
      toast({
  ...
  return (
    <Flex direction={['column', 'column', 'row']} bg={bg} color={color} height="100vh">
      {/* Main content area */}
      <Box flex="1" p={8} color="">
        ...
        <Tabs variant="soft-rounded" colorScheme="purple">
          <TabList>
            <Tab>Generated Images</Tab>
            <Tab>Favorite Images</Tab>
          </TabList>
          <TabPanels>
            <TabPanel>
              ...
              <SimpleGrid columns={[1, 2, 3]} spacing={4} ref={generatedImagesRef}>
                  ...
                  >
                    ...
                    <Flex justifyContent="center" mt={2}>
                      <Button
                        ...
                        size="sm"
                        mr={2}
                        onClick={(e) => { e.stopPropagation(); handleDownloadImage(image); }}
                      >
                        Download
                      </Button>
                      <Button
                        ...
                        size="sm"
                        mr={2}
                        onClick={(e) => { e.stopPropagation(); handleCopyImageURL(image); }}
                      >
                        Copy URL
                      </Button>
                      <Button
                        ...
                        colorScheme="yellow"
                        size="sm"
                        onClick={(e) => { e.stopPropagation(); handleFavoriteImage(image); }}
                      >
                        Favorite
                      </Button>
                    </Flex>
            ...
            <TabPanel>
              <Heading size="md" mb={4}>Favorite Images:</Heading>
              <SimpleGrid columns={[1, 2, 3]} spacing={4}>
                {favoriteImages.map((image) => (
                  <Box
                    ...
                    _hover={{ transform: 'scale(1.05)', boxShadow: 'xl' }}
                  >
                    <Image src={image.imageUrl} alt={`Favorite Image ${image.id}`} onClick={() => handleImageClick(image.imageUrl)} />
                    <Flex justifyContent="center" mt={2}>
                      <Button
                        leftIcon={<DownloadIcon />}
                        colorScheme="purple"
                        size="sm"
                        mr={2}
                        onClick={(e) => { e.stopPropagation(); handleDownloadImage(image.imageUrl); }}
                      >
                        Download
                      </Button>
                      <Button
                        ...
                        size="sm"
                        mr={2}
                        onClick={(e) => { e.stopPropagation(); handleCopyImageURL(image.imageUrl); }}
                      >
                        Copy URL
                      </Button>
                      <Button
                        ...
                        colorScheme="red"
                        size="sm"
                        onClick={(e) => { e.stopPropagation(); handleDeleteFavoriteImage(image.id, image.storagePath); }}
                      >
                        Delete
                      </Button>
                    </Flex>
          ...
          <form onSubmit={handleGenerateImage}>
            <Flex mb={4} direction="column">
              ...
              <Flex mb={2} alignItems="center">
                ...
                <Select value={aspectRatio} onChange={(e) => setAspectRatio(e.target.value)} bg={sidebarBg} borderColor="gray.600" color={color}>
              ...
              <Flex mb={2} alignItems="center">
                ...
                <Select value={model} onChange={(e) => setModel(e.target.value)} bg={sidebarBg} borderColor="gray.600" color={color}>
              ...
              <Flex mb={2} alignItems="center">
                ...
                <Select value={outputFormat} onChange={(e) => setOutputFormat(e.target.value)} bg={sidebarBg} borderColor="gray.600" color={color}>
      ...
      >
        <Flex direction="column" align="center">
          <Image src={modalImage || ''} alt="Generated Image" mb={4} borderRadius="md" boxShadow="lg" />
